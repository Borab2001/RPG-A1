...
public class Game {
    ...
    public static enum Status {START_COMBAT, HERO_TURN, ENEMY_TURN, END_GAME,WINNING }
    ...
    public List<String> getHeroesStatus() {
        ...
        for (Hero hero: this.heroes) {
            if(hero instanceof SpellCaster){
                heroesStatus.add( hero.getClass().getSimpleName()
                        + "( hp: " + hero.getLifePoints() + ", mana: " + ((SpellCaster)hero).getManaPoints() + ")");
            }
            else if(hero instanceof Hunter){
                heroesStatus.add( hero.getClass().getSimpleName()
                        + "( hp: " + hero.getLifePoints() + ", arrow: " + ((Hunter)hero).getArrows() + ")");
            }
            else{
                heroesStatus.add( hero.getClass().getSimpleName()
                        + "( hp: " + hero.getLifePoints() + ")");
            }
    ...
    public void startNextFighterTurn() {
        ...
        if (this.heroes.size() == 0) {
            ...
            this.status = Game.Status.WINNING;
            //generateCombat();
        } else {
    ...
    public Hero startHeroTurn(int i, boolean healAction, boolean defenseAction) {
        ...
        if(defenseAction){
            ...
            if(currentFighter instanceof Hunter && ((Hunter) currentFighter).getArrows() <= 0) {
                ...
                Fighter ennemy = this.enemies.get(i);
